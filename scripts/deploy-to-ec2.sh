...
if [ "$DEPLOY_MODE" = "tag" ]; then
    echo "Deploying tag $TAG from git repository..."
    
    # Checkout tag on remote server
    ssh -i $SSH_KEY $EC2_USER@$EC2_HOST "cd /home/ubuntu/mobiledroid && git fetch --tags && git checkout $TAG"
    
    if [ $? -ne 0 ]; then
        echo "Error: Failed to checkout tag $TAG on remote server"
        exit 1
    fi
